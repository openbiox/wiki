...
1. 讨论维持社区活跃度、增加成员参与度的方案（10-15 min）
2. 是否采用 [community.openbiox.org](http://community.openbiox.org) 论坛 + 微信的方式开展活动。包括议题讨论、项目开发与进展、成员活动、问答等，并建立相应论坛权限分配体系？（5-7 min）
3. 社区贡献统计方式是否需要进行修订为基于实践项目的贡献积分制：即以完成的具体项目为基础，在项目完成之后，向全体成员进行申报并进行投票、分级和总贡献积分的确定，并由项目负责人来分配所获取项目积分。如果某个项目得到了不错的长期维护和功能更新，可定期进行额外积分奖励。（5-7 min）
4. 奖励和激励方案细化：1. 每月对外发布积分贡献榜（可使用化名或者实名），在知乎和微信公众平台进行发布。2. 使用积分可以兑换书籍或者现金奖励 3. 使用积分获取社区电子资源的下载权限 4. 使用积分申请社区共享资源的使用（场地、服务器等）(5-7 min)
5. 目前已有项目的状态盘点（活跃、暂停、终止），并相应进行总结、删减、重启 （7-10 min）
6. 重启或者启动可以进一步扩大社区影响力的项目：翻译项目；生物信息学基础书籍编写；自然语言处理实践项目；bapi、bget；xxx 100 days；......（10-15 min）
7. 是否需要修订成员退出方案：退出机制修订为自愿退出，在长时间未参与或贡献任何项目时进行标注。（5-7 min）
8. 例会时间是否需要进行调整：由两周一次修订为每月一次，如果有重要事务需要及时讨论可由委员会成员提前一星期发布额外会议提醒。（5-7 min）
9. 常务委员会成员更新：1. 是否有不再愿意担任委员会职务的成员；2. 筛选新一批成员进入委员会（10-15min）
10. 11月是否需要进行一次线上和线下交流会，负责团队人选确定（10-15min）
...
1. 增加线下交流活动（\*\*婷）
2. 可以以 openbiox 微信公众号为平台 推送某些项目的进度 或对某些文章的解读 或者对某些文章中新分析技术的使用经验分享（\*\*云）
3. 微信的推送相对比较正式，在简书或者别的平台感觉帖子相对非正式一些，占用时间也少。我们可以鼓励大家写一下心得或者教程，然后比较精品的可以考虑微信推送（\*逸）
4. 可以专门建个仓库用于微信投稿，用 Pull request。投稿人通过 issue 提交稿件申请，然后经初步审核过通过合并请求提交稿件，经过多人评审后发布到微信公众号（\*\*翔）
5. 动态记录和展示成员活动度（\*\*翔）
6. 整合成员之前发布的文章/博客在 openbiox 社区进行发布（\*逸）
7. 做大 openbiox 的公共宣传渠道：知乎/微信公众平台

【议题2】

1. 使用微信进行即时交流，暂停钉钉的使用
2. 提升[commuity.openbiox.org](https://community.openbiox.org/) 论坛的使用力度，论坛用于比较正式的协作，包括代码交流，计划进展等
...
1. 定期/实时发布社区用户的贡献榜
2. 贡献和积分挂钩，同时使用积分用于兑换奖励

【议题5】

1. 已完成：Cookbook for R 翻译
2. 维护中：XenaShiny、bget、bapi、butils、awosome-bioinformatics、bioinformatics-100-days、golang-100-days、openbiox主页、wiki、写作素材协作
3. 暂停状态：微云书籍资源共享和分类、gists、bioshiny、playcloud、disease knowledge-base and cloud platform、NLP
...
退出机制修订为自愿退出，在长时间未参与或贡献任何项目时进行标注

【议题8】

由两周一次修订为每月一次，如果有重要事务需要及时讨论可由委员会成员提前一星期发布额外会议提醒

【议题9】

全体成员中征集或提名志愿者并由现委员会成员投票通过
